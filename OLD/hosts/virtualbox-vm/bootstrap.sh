...
parted /dev/sda mkpart primary "${_starts_at}MiB" "${_ends_at}MiB" set 1 esp on && sleep 1
...
printinfo "\n"
printinfo "+ --------------------------------------------------------------- +"
printinfo "| Formatting /dev/sda and setting up LUKS encrypted partition |"
printinfo "+ --------------------------------------------------------------- +"
mkfs.fat -F32 /dev/sda1 && sleep 1
mkfs.f2fs -f /dev/sda2 && sleep 1
cryptsetup --verbose luksFormat /dev/sda3 && sleep 1
cryptsetup open /dev/sda3 root && sleep 1
mkfs.f2fs -O encrypt -f /dev/mapper/root && sleep 1
mkfs.f2fs -O encrypt -f /dev/sda4 && sleep 1
...
mount /dev/sda2 "$_rootmnt"/boot
mkdir -p "$_rootmnt"/boot/efi
mount /dev/sda1 "$_rootmnt"/boot/efi

mkdir -p "$_rootmnt"/media/vol1
mount /dev/sda4 "$_rootmnt"/media/vol1
